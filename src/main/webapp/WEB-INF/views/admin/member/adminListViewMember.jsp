			...
			<div id="layoutSidenav_content">
				<main>
					<div class="container-fluid px-4">
						...
						<div class="card mb-4">
							<div class="card-body">
								해당 페이지에서는 일반 회원을 조회 및 삭제 처리 가능합니다.
								<a target="_blank" href="adminRegistForm.me">계정 추가의 경우 여기를 클릭하세요.</a>
							</div>
						</div>
						<div class="card mb-4">
							...
							<div class="card-body">
								<table class="card-table" align="center">
									...
									<thead>
										<tr>
											<th colspan="5" style="border-left: 1px solid #5271FF; border-right: 1px solid #5271FF;">계정정보</th>
											<th colspan="4" style="border-right: 1px solid #5271FF;">가입정보</th>
										</tr>
										<tr>
		...
		<script>
			...
			function onLoad(page) {
				$.ajax({
					data : { "page": page },
					type : "get",
					url : "adminListLoad.me",
					success : function(col) {
						if (col=="none") { $("#moreLoad").attr("disabled", true); $("#tb").html($("#tb").html() + "<tr><td colspan='7' style='border-top: 1px solid #5271FF; border-bottom: 1px solid #5271FF;'>더 이상 표기할 정보가 없습니다.</td></tr>"); }
						else { $("#tb").html($("#tb").html() + col); }
					},
					error: function() { alert("로딩에 실패함"); }
				});
			};
			function onDelete(e, memberNo) {
				$.ajax({
					data : { "memberNo": memberNo },
					type : "post",
					url : "adminDelete.me",
					success : function(result) { $(e).parent().html("<button type='button' class='btn btn-info' onclick='onRestore(this, " + memberNo + ");'>복원시키기</button>"); },
					error: function() { alert("삭제에 실패함"); }
				});
			};
			function onRestore(e, memberNo) {
				$.ajax({
					data : { "memberNo": memberNo },
					type : "post",
					url : "adminRestore.me",
					success : function(result) { $(e).parent().html("<button type='button' class='btn btn-danger' onclick='onDelete(this, " + memberNo + ");'>탈퇴시키기</button>"); },
