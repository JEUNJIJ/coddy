...
<html>
	<head>
	<meta charset="UTF-8">
	<title>코드 리뷰 게시판 리스트 보기 페이지</title>
	<style>
	...
	<body class="sb-nav-fixed">
			...
			<div id="layoutSidenav_content">
				<main>
					<div class="container-fluid px-4">
						...
						<div class="card mb-4">
							<div class="card-body">
								해당 페이지에서는 코드 리뷰 게시판 리스트를 출력합니다. 필요한 경우 삭제를 해주세요.
							</div>
						</div>
						<div class="card mb-4">
		...
		<script>
			...
			function onDelete(e, cboardNo) {
				$.ajax({
					data : { "cboardNo": cboardNo },
					type : "post",
					url : "adminDelete.co",
					success : function(result) { $(e).parent().html("<button type='button' class='btn btn-info' onclick='onRestore(this, " + cboardNo + ");'>복원시키기</button>"); },
					error: function() { alert("삭제에 실패함"); }
				});
			};
			function onRestore(e, cboardNo) {
				$.ajax({
					data : { "cboardNo": cboardNo },
					type : "post",
					url : "adminRestore.co",
					success : function(result) { $(e).parent().html("<button type='button' class='btn btn-danger' onclick='onDelete(this, " + cboardNo + ");'>탈퇴시키기</button>"); },
